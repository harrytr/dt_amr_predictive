...
def _sim_dirs_for_condition(condition_dir: Path) -> List[Path]:
    # Supports both the old layout freq_X/sim_000 and the paired layout
    # freq_X/{endog,import}/sim_000.
    return sorted(
...
def main() -> int:
    ...
    condition_dirs = sorted([p for p in BASE.iterdir() if p.is_dir() and p.name.startswith("freq_")])
    total_all = 0
    ...
    for condition_dir in condition_dirs:
        ...
        total_all += total
        print(f"FREQ_DONE {condition_dir.name}: sims={len(sims)} pt_files={total} out={out_dir}", flush=True)
