...
BASE_OUTPUT_DIR = Path("synthetic_endog_import_step6_freq_v1")

# Step 6 is a screening-frequency stress test for the predictive task.  The
# default profile now mirrors the canonical Step 1 train split so that Step 6
# changes the screening-frequency intervention without also weakening the
...
DEFAULT_FREQS = [3, 7, 14]
DEFAULT_N_SIMS_PER_REGIME = 10
DEFAULT_STEP6_NUM_DAYS = 60
DEFAULT_STEP6_SCREEN_RESULT_DELAY_DAYS = 2
...
def run_one(
    ...
    *,
    ...
    cmd: List[str] = [
        ...
        str(int(screen_result_delay_days)),
    ...
    print(
        f"RUN_FREQ regime={regime_name} freq={freq} seed={seed} num_days={num_days}:",
        " ".join(cmd),
        flush=True,
    )
...
def main() -> int:
    freqs = _parse_int_list_env("DT_STEP6_FREQS", DEFAULT_FREQS, min_value=1)
    ...
    screen_result_delay_days = _env_int("DT_STEP6_FREQ_DELAY_DAYS", DEFAULT_STEP6_SCREEN_RESULT_DELAY_DAYS, min_value=0)
    ...
    _write_manifest(
        ...
        num_days=num_days,
        screen_result_delay_days=screen_result_delay_days,
    ...
    print(
        "STEP6_FREQ_CONFIG "
        f"profile={profile} freqs={freqs} n_sims_per_regime={n_sims_per_regime} "
        f"num_days={num_days} fixed_delay_days={screen_result_delay_days} "
    ...
    for freq in freqs:
        fdir = BASE_OUTPUT_DIR / f"freq_{freq}"
        fdir.mkdir(parents=True, exist_ok=True)

        for regime_name, regime in regimes.items():
            regime_dir = fdir / regime_name
            ...
            for r in range(n_sims_per_regime):
                ...
                seed = 9500 + freq * 100 + seed_offset + r
                rc = run_one(
                    out,
                    seed=seed,
                    freq=freq,
                ...
                if rc != 0:
                    print(
                        f"FAILED freq={freq} regime={regime_name} sim={r} rc={rc}",
                        flush=True,
                    )
                    return rc

    print("STEP6C_DONE: generated paired screening frequency grid", flush=True)
    return 0
